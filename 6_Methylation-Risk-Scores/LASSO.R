...
xfactors <- model.matrix(T2D_status ~ Age + Sex2 + Ethnicity, data=beta_sig_SPHS_ALL_tv3)[, -1]
x <- as.matrix(data.frame(xfactors,beta_sig_SPHS_ALL_tv3[,c(6:137)]))

beta_merge_meth = data.frame(x)
beta_merge_meth_tv3 = data.frame(beta_sig_SPHS_ALL_tv3$T2D_status,beta_merge_meth)
